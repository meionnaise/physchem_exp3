...
#fe
fe_labels = np.array([1.8, 1.6, 1.2, 0.8, 0.4, 0.2, 0])
fe_labels /= 1000
...
cu_labels = np.array([0.18, 0.16, 0.12, 0.08, 0.04, 0.02, 0])
...
plt.scatter(q1_fexaxis, q1_feyaxis, color = "#fda0cc", marker = 'o', )
xerrb = np.std(q1_fexaxis) #multiply by two or nah bc theyre on either side already
yerrb = np.std(q1_feyaxis)
plt.errorbar(q1_fexaxis, q1_feyaxis, xerr = xerrb , yerr = yerrb, color = "#fda0cc", ls = "none")
...
plt.scatter(q1_cuxaxis, q1_cuyaxis, color = "#fda0cc", marker = 'o', )
xerrb = np.std(q1_cuxaxis) #multiply by two or nah bc theyre on either side already
yerrb = np.std(q1_cuyaxis)
plt.errorbar(q1_cuxaxis, q1_cuyaxis, xerr = xerrb , yerr = yerrb, color = "#fda0cc", ls = "none")
...
plt.rcParams['axes.facecolor'] = "#FFE1EF"
plt.figure(facecolor="#FFE1EF")
#plot normalised emission
i = 0
for sample in fe:
    sample = normalise_col(sample)
    plt.plot(sample[:,0], sample[:,1], label = fe_labels[i])
    i += 1
plt.plot(rubpy_emission_norm[:, 0], rubpy_emission_norm[:,1], label = "Rubpy Emission", linestyle = "--", color = "#fda0cc")
plt.xlabel("Wavelength (nm)")
plt.ylabel("Emission intensity")
plt.title("Emission intensity of Fe3+ solutions compared to Rubpy")
plt.grid(color = 'w', linestyle = '-', linewidth = 0.5)
plt.legend(title = "[Fe3+] (M)")
plt.savefig('part1_discussionq2_fe.png')
...
for sample in cu:
    sample = normalise_col(sample)
    plt.plot(sample[:,0], sample[:,1], label = cu_labels[i])
    i += 1
plt.plot(rubpy_emission_norm[:, 0], rubpy_emission_norm[:,1], label = "Rubpy Emission", linestyle = "--", color = "#fda0cc")
plt.xlabel("Wavelength (nm)")
plt.ylabel("Emission intensity")
plt.title("Emission intensity of Cu2+ solutions compared to Rubpy")
plt.grid(color = 'w', linestyle = '-', linewidth = 0.5)
plt.legend(title = "[Cu2+] (M)")
plt.savefig('part1_discussionq2_cu.png')
